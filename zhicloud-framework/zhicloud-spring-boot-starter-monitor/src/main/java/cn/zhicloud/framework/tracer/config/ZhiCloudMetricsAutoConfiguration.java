package cn.zhicloud.framework.tracer.config;

import cn.zhicloud.framework.tracer.core.metrics.DruidMetricsBinder;
...
@ConditionalOnProperty(prefix = "zhicloud.metrics", value = "enable", matchIfMissing = true) // 允许使用 zhicloud.metrics.enable=false 禁用 Metrics
public class ZhiCloudMetricsAutoConfiguration {
    ...
    public DruidMetricsBinder druidMetricsBinder(DataSource dataSource, MeterRegistry meterRegistry) {
        ...
        if (dataSource instanceof DynamicRoutingDataSource dynamicDs) {
            ...
            log.info("[ZhiCloudMetricsAutoConfiguration] Druid 指标绑定器已注册动态数据源: {}", inner.keySet());
        } else {
            // 兜底：单数据源场景
            binder.registerDataSource("default", dataSource);
            log.info("[ZhiCloudMetricsAutoConfiguration] Druid 指标绑定器已注册默认数据源: {}", dataSource.getClass().getSimpleName());
        }
        // P1-4：显式调用 bindTo，确保指标立即注册到 MeterRegistry
        // （MeterRegistryPostProcessor 可能在本 Bean 创建前已处理过 MeterRegistry，导致自动绑定失效）
        binder.bindTo(meterRegistry);
        log.info("[ZhiCloudMetricsAutoConfiguration] Druid 指标已绑定到 MeterRegistry");
        return binder;
    }
