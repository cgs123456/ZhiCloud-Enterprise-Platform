package cn.zhicloud.framework.tenant.config;

import cn.zhicloud.framework.common.biz.system.tenant.TenantCommonApi;
import cn.zhicloud.framework.common.enums.WebFilterOrderEnum;
import cn.zhicloud.framework.mybatis.core.util.MyBatisUtils;
import cn.zhicloud.framework.redis.config.ZhiCloudCacheProperties;
...
import cn.zhicloud.framework.tenant.core.aop.TenantIgnoreAspect;
import cn.zhicloud.framework.tenant.core.db.TenantDatabaseInterceptor;
import cn.zhicloud.framework.tenant.core.job.TenantJobAspect;
import cn.zhicloud.framework.tenant.core.mq.rabbitmq.TenantRabbitMQInitializer;
import cn.zhicloud.framework.tenant.core.mq.redis.TenantRedisMessageInterceptor;
import cn.zhicloud.framework.tenant.core.mq.rocketmq.TenantRocketMQInitializer;
import cn.zhicloud.framework.tenant.core.redis.TenantRedisCacheManager;
import cn.zhicloud.framework.tenant.core.security.TenantSecurityWebFilter;
import cn.zhicloud.framework.tenant.core.service.TenantFrameworkService;
import cn.zhicloud.framework.tenant.core.service.TenantFrameworkServiceImpl;
import cn.zhicloud.framework.tenant.core.web.TenantContextWebFilter;
import cn.zhicloud.framework.tenant.core.web.TenantVisitContextInterceptor;
...
@AutoConfiguration
@ConditionalOnProperty(prefix = "zhicloud.tenant", value = "enable", matchIfMissing = true) // 允许使用 zhicloud.tenant.enable=false 禁用多租户
@EnableConfigurationProperties(TenantProperties.class)
public class ZhiCloudTenantAutoConfiguration {
    ...
    @ConditionalOnClass(name = "cn.zhicloud.framework.mq.redis.core.interceptor.RedisMessageInterceptor")
    ...
    public RedisCacheManager tenantRedisCacheManager(RedisTemplate<String, Object> redisTemplate,
                                                     ...
                                                     ZhiCloudCacheProperties zhicloudCacheProperties,
                                                     TenantProperties tenantProperties,
                                                     CacheProperties cacheProperties) {
        // 创建 RedisCacheWriter 对象
        RedisConnectionFactory connectionFactory = Objects.requireNonNull(redisTemplate.getConnectionFactory());
        RedisCacheWriter cacheWriter = RedisCacheWriter.nonLockingRedisCacheWriter(connectionFactory,
                BatchStrategies.scan(zhicloudCacheProperties.getRedisScanBatchSize()));
        // 创建带租户后缀的 Key 前缀：在计算缓存 Key 时动态拼接租户编号，
        // 无需按租户分裂 Cache 实例，避免内存中 Cache 对象无限增长
        RedisCacheConfiguration tenantCacheConfiguration = redisCacheConfiguration.computePrefixWith(
                TenantRedisCacheManager.buildTenantCacheKeyPrefix(
                        cacheProperties.getRedis().getKeyPrefix(), tenantProperties.getIgnoreCaches()));
        // 创建 TenantRedisCacheManager 对象
        TenantRedisCacheManager cacheManager = new TenantRedisCacheManager(cacheWriter, tenantCacheConfiguration);
    ...
    @Configuration(proxyBeanMethods = false)
    @ConditionalOnClass(name = "cn.zhicloud.framework.quartz.core.handler.JobHandler")
