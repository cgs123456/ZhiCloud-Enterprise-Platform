package cn.zhicloud.framework.mq.redis.core.job;
...
public class RedisPendingMessageResendJob {
    ...
    /**
     * 死信队列 stream key 后缀（原 topic + ":dlq"）
     */
    ...
    /**
     * 毒消息判定阈值：pending 消息投递次数超过该值时转入死信队列并 ACK，不再重投。
     * 可通过 zhicloud.mq.stream.max-delivery-count 配置，默认 5。
     */
    @Value("${zhicloud.mq.stream.max-delivery-count:5}")
    ...
    public RedisPendingMessageResendJob(List<AbstractRedisStreamMessageListener<?>> listeners,
                                        ...
                                        String resendLockKey) {
        this.listeners = listeners;
        this.redisTemplate = redisTemplate;
        this.redissonClient = redissonClient;
        this.resendLockKey = resendLockKey;
    }
        ...
        listeners.forEach(listener -> {
            ...
            pendingMessagesPerConsumer.forEach((consumerName, pendingMessageCount) -> {
                ...
                pendingMessages.forEach(pendingMessage -> {
                    ...
                    // 毒消息判定：投递次数超过阈值，转存死信队列后 ACK，避免无限重投
                    if (pendingMessage.getTotalDeliveryCount() > maxDeliveryCount) {
                        String dlqStreamKey = listener.getStreamKey() + DLQ_STREAM_SUFFIX;
                        // 1. XADD 到死信 stream（保留原消息内容）
                        ops.add(StreamRecords.newRecord()
                                .ofObject(records.get(0).getValue())
                                .withStreamKey(dlqStreamKey));
                        // 2. XACK 原消息
                        ops.acknowledge(listener.getStreamKey(), listener.getGroup(), pendingMessage.getIdAsString());
                        log.error("[processPendingMessage][消息({}) 投递次数({}) 超过上限({})，已转存死信队列({})]",
                                pendingMessage.getIdAsString(), pendingMessage.getTotalDeliveryCount(),
                                maxDeliveryCount, dlqStreamKey);
                        return;
                    }
                    // 重新投递消息
