package cn.zhicloud.framework.xss.core.clean;
...
public class JsoupXssCleaner implements XssCleaner {
    ...
    /**
     * 危险的 style 属性值模式：url(、expression(、javascript:、@import，命中则移除该属性
     */
    private static final Pattern INVALID_STYLE_PATTERN =
            Pattern.compile("(?i)(url\\(|expression\\(|javascript:|@import)");
    ...
    private Safelist buildSafelist() {
        // 使用 jsoup 提供的默认的
        Safelist relaxedSafelist = new Safelist(Safelist.relaxed()) {

            @Override
            public boolean isSafeAttribute(String attributeName, Element el, Attribute attr) {
                // 安全加固：style 属性值包含 url( / expression( / javascript: / @import 等危险内容时，直接移除该属性
                // 避免形如 <img STYLE="background-image:url(javascript:alert('XSS'))"> 的注入风险
                if ("style".equalsIgnoreCase(attributeName)
                        && INVALID_STYLE_PATTERN.matcher(attr.getValue()).find()) {
                    return false;
                }
                return super.isSafeAttribute(attributeName, el, attr);
            }
