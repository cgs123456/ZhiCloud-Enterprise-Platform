package cn.zhicloud.framework.security.core.util;
...
public class SecurityFrameworkUtils {
    ...
    /**
     * WebSocket 握手路径前缀。原因是浏览器 WebSocket 握手无法自定义 Header，只能通过 URL 参数传递 Token
     */
    private static final String WEBSOCKET_TOKEN_PATH_PREFIX = "/infra/ws";
    ...
    public static String obtainAuthorization(HttpServletRequest request,
        ...
        if (StrUtil.isEmpty(token)) {
            // 安全加固：Token 不再允许任意 URL 传参（避免通过访问日志、Referer、浏览器历史泄露），
            // 仅 WebSocket 握手（/infra/ws 前缀）因无法自定义 Header 而保留 query 参数兼容
            HttpServletRequest currentRequest = ServletUtils.getRequest() != null ? ServletUtils.getRequest() : request;
            if (currentRequest.getRequestURI().startsWith(WEBSOCKET_TOKEN_PATH_PREFIX)) {
