package cn.zhicloud.framework.mybatis.core.mapper;

import cn.hutool.core.collection.CollUtil;
import cn.zhicloud.framework.common.pojo.PageParam;
import cn.zhicloud.framework.common.pojo.PageResult;
import cn.zhicloud.framework.common.pojo.SortablePageParam;
import cn.zhicloud.framework.common.pojo.SortingField;
...
public interface BaseMapperX<T> extends MPJBaseMapper<T> {
    ...
    default T selectFirstOne(SFunction<T, ?> field, Object value) {
        // 通过分页查询实现 LIMIT 1，避免全表加载，且数据库方言无关
        PageParam pageParam = new PageParam();
        pageParam.setPageNo(1);
        pageParam.setPageSize(1);
        List<T> list = selectPage(pageParam, new LambdaQueryWrapper<T>().eq(field, value)).getList();
    ...
    default T selectFirstOne(SFunction<T, ?> field1, Object value1, SFunction<T, ?> field2, Object value2) {
        ...
        pageParam.setPageNo(1);
        pageParam.setPageSize(1);
        List<T> list = selectPage(pageParam,
                new LambdaQueryWrapper<T>().eq(field1, value1).eq(field2, value2)).getList();
        return CollUtil.getFirst(list);
    }
    ...
    default T selectFirstOne(SFunction<T,?> field1, Object value1, SFunction<T,?> field2, Object value2,
        ...
        pageParam.setPageNo(1);
        pageParam.setPageSize(1);
        List<T> list = selectPage(pageParam, new LambdaQueryWrapper<T>()
                .eq(field1, value1).eq(field2, value2).eq(field3, value3)).getList();
        return CollUtil.getFirst(list);
    }
    ...
    default T selectLastOne(LambdaQueryWrapper<T> queryWrapper) {
        // 通过「总数 + 跳到最后一页（pageSize=1）」实现 LIMIT 1 取最后一条，保持原语义且数据库方言无关
        Long count = selectCount(queryWrapper);
        if (count == null || count <= 0) {
            return null;
        }
        PageParam pageParam = new PageParam();
        pageParam.setPageNo(count.intValue());
        pageParam.setPageSize(1);
        List<T> list = selectPage(pageParam, queryWrapper).getList();
