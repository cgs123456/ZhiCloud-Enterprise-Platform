...
- `iceberg.rest-catalog.warehouse=s3://yudao-warehouse`：Iceberg 仓库路径
- `iceberg.rest-catalog.s3.endpoint=http://yudao-minio:9000`：MinIO S3 端点

### 3. 初始化 MinIO Bucket

启动 MinIO 后，访问 `http://localhost:9001`（用户名/密码：minioadmin/minioadmin123），
创建 `yudao-warehouse` bucket。
...
DATALAKE_WAREHOUSE_PATH=s3://yudao-warehouse
```
...
| 配置项 | 环境变量 | 默认值 | 说明 |
|--------|----------|--------|------|
| `yudao.datalake.enabled` | `DATALAKE_ENABLED` | `false` | 是否启用数据湖仓模块 |
| `yudao.datalake.catalog-uri` | `DATALAKE_CATALOG_URI` | `http://trino:8080` | Trino REST API 地址 |
| `yudao.datalake.warehouse-path` | `DATALAKE_WAREHOUSE_PATH` | `s3://yudao-warehouse` | 归档存储路径 |
| `yudao.datalake.retention-days` | `DATALAKE_RETENTION_DAYS` | `365` | 默认归档保留天数 |
| `yudao.datalake.batch-size` | - | `10000` | 归档批处理大小 |
| `yudao.datalake.archive-tables` | - | `[]` | 需要归档的表列表 |
...
├── pom.xml
├── README.md
└── src/main/java/cn/zhicloud/yudao/module/datalake/
    ├── DataLakeApplication.java          # 模块入口标记类
    ├── package-info.java                 # Spring Modulith 模块声明
    ├── config/
    │   ├── DataLakeProperties.java       # 配置属性（yudao.datalake.*）
...
7. **归档任务调度**：对接 yudao-spring-boot-starter-job / XXL-Job
