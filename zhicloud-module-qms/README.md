...
`zhicloud-module-qms` 是 zhicloud 项目的业务模块（Spring Modulith 声明 A3），实现制造企业质量管理的核心流程：
...
模块依赖 `zhicloud-module-system` / `zhicloud-module-infra`，并通过 `zhicloud-module-mes` / `zhicloud-module-erp` 接收外部触发（详见 §3 业务边界）。
...
### 3.3 与 zhicloud-module-ai 的关系

- 当前 QMS 模块本身不依赖 `zhicloud-module-ai`
- 规划中将通过 Spring AI `@Tool` 注解暴露 MCP 工具给 zhicloud-module-ai 的 ReAct Agent / 多 Agent 编排调用（详见 §7 MCP 工具）
...
└── src/main/java/cn/zhicloud/zhicloud/module/qms/
...
> 建表脚本：`sql/mysql/qms.sql`，所有表均含 `creator/create_time/updater/update_time/deleted/tenant_id` 字段（继承 zhicloud 框架 `TenantBaseDO`）。
...
> 以下 4 个 MCP 工具为本模块与 `zhicloud-module-ai` ReAct Agent / 多 Agent 编排对接的**规划能力**。当前 QMS 模块未依赖 `zhicloud-module-ai`，待后续通过 Spring AI `@Tool` 注解暴露后，可被 `zhicloud-module-ai-multiagent` 的 `ReActAgent` 自动发现并调用。
...
启用方式（待实现）：在对应 ServiceImpl 方法上添加 `@Tool(description = "...")` 注解，并在 zhicloud-server 中通过 `MethodToolCallbackProvider` 注册为 Spring AI ToolCallback。

## 9. 错误码段

使用 `1-040-xxx-xxx` 段（与 zhicloud-module-ai 共享 1-040 前缀，但通过子段区分）：
...
- zhicloud-module-mes（制造执行系统）：见项目根目录
- zhicloud-module-erp（企业资源计划）：见项目根目录
