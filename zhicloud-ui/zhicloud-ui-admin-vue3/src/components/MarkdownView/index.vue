...
// 属性值转义：代码文本含单引号等字符时直接嵌入 data-copy 属性会逃逸注入，需先转义
...
const md = new MarkdownIt({
  highlight: function (str, lang) {
    if (lang && hljs.getLanguage(lang)) {
      try {
        const copyHtml = `<div id="copy" data-copy='${escapeAttr(str)}' style="position: absolute; right: 10px; top: 5px; color: #fff;cursor: pointer;">复制</div>`
