...
export class AES {
  ...
  static encrypt(data: string, key: string): string {
    try {
      ...
      // 输出格式：Base64(IV(16字节) || 密文)，与后端 AES/CBC/PKCS5Padding 约定一致
  ...
  static decrypt(encryptedData: string, key: string): string {
    try {
      ...
      const keyUtf8 = CryptoJS.enc.Utf8.parse(key)
      // 输入格式：Base64(IV(16字节) || 密文)，先 Base64 解码，取前 16 字节作为 IV
      const rawData = CryptoJS.enc.Base64.parse(encryptedData)
      if (!rawData || rawData.sigBytes <= 16) {
        throw new Error('AES 解密数据格式错误，长度不足')
      }
