...
/** 初始化腾讯地图 */
// 腾讯地图选点组件通过 postMessage 回传位置；持引用以便卸载时移除，避免重复挂载累积监听器
const tencentLbsMessageHandler = (event: MessageEvent) => {
...
const initTencentLbsMap = async () => {
  ...
  window.addEventListener('message', tencentLbsMessageHandler, false)
...
onUnmounted(() => {
  window.removeEventListener('message', tencentLbsMessageHandler)
})
</script>
