<!--
  MES 点检保养项目弹窗选择器（支持单选/多选）

  Props:
    multiple — true 多选（checkbox），false 单选（radio）；默认 true
    subjectType — 可选，过滤指定项目类型
  Events:
    selected(rows: DvSubjectVO[]) — 确认选择后触发，单选时数组长度为 1
  Expose:
    open(selectedIds?: number[]) — 打开弹窗，可传入已选 ID 用于预选高亮
-->
<template>
  <Dialog title="点检保养项目选择" v-model="dialogVisible" width="70%">
    <ContentWrap>
...
defineOptions({ name: 'DvSubjectSelectDialog' })

const props = withDefaults(
  defineProps<{
    multiple?: boolean // true 多选（checkbox），false 单选（radio）
    subjectType?: number // 可选：过滤指定项目类型
  }>(),
  {
    multiple: true
  }
...
const emit = defineEmits<{
  selected: [rows: DvSubjectVO[]]
}>()

const dialogVisible = ref(false) // 弹窗是否展示
const loading = ref(false) // 列表加载中
const list = ref<DvSubjectVO[]>([]) // 列表
const total = ref(0) // 总条数

// ==================== 选中状态 ====================
const tableRef = ref() // 表格 Ref
const selectedRows = ref<DvSubjectVO[]>([]) // 多选模式：选中行
const selectedRadioId = ref<number>() // 单选模式：选中 ID
const currentRadioRow = ref<DvSubjectVO>() // 单选模式：选中行对象
const preSelectedIds = ref<number[]>([]) // 打开弹窗时传入的已选 ID

/** 多选：checkbox 变化 */
const handleSelectionChange = (rows: DvSubjectVO[]) => {
  if (props.multiple) {
...
/** 单选：radio 变化 */
const handleRadioChange = (row: DvSubjectVO) => {
  currentRadioRow.value = row
}

/** 单击行：单选模式下点击整行即选中（降低操作成本），多选不处理（避免和 dblclick 冲突） */
const handleRowClick = (row: DvSubjectVO) => {
  if (props.multiple) {
    return
  }
...
const handleRowDblClick = (row: DvSubjectVO) => {
  if (props.multiple) {
...
const queryParams = reactive({
  pageNo: 1, // 页码
  pageSize: 10, // 每页条数
  code: undefined as string | undefined, // 项目编码
  name: undefined as string | undefined, // 项目名称
  type: undefined as number | undefined, // 项目类型
...
const open = async (selectedIds?: number[]) => {
  ...
  queryParams.code = undefined
  queryParams.name = undefined
  queryParams.type = props.subjectType
  queryParams.status = CommonStatusEnum.ENABLE
  queryParams.pageNo = 1
  // 清空上一次的选中状态
  selectedRows.value = []
