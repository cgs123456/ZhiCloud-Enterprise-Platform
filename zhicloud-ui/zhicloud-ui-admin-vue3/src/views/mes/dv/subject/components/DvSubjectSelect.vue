<!--
  MES 点检保养项目选择器：只读输入框 + 点击弹窗选择

  交互：显示为只读 el-input，点击打开弹窗（单选模式）进行选择
  Props:
    modelValue  — 绑定的项目 ID（v-model）
    disabled    — 是否禁用
  ...
  Events:
    ...
    change(item)      — 选中项目变化时触发，传递完整 DvSubjectVO（清空时为 undefined）
-->
<template>
  <div
    v-bind="attrs"
...
defineOptions({ name: 'DvSubjectSelect', inheritAttrs: false })

const props = withDefaults(
  defineProps<{
    modelValue?: number // 绑定的项目 ID
...
// ==================== 名称回显 ====================
const selectedItem = ref<DvSubjectVO | undefined>() // 当前选中的项目对象

/** 输入框显示文本：只展示项目名称，保持简洁 */
...
const resolveItemById = async (id: number | undefined) => {
  ...
  try {
    selectedItem.value = await DvSubjectApi.getSubject(id)
  } catch (e) {
    console.error('[DvSubjectSelect] resolveItemById failed:', e)
  }
}
...
/** 弹窗选中回调 */
const handleSelected = (rows: DvSubjectVO[]) => {
