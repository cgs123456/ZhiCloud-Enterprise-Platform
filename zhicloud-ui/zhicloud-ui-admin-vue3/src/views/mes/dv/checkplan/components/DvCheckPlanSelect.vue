<!--
  MES 点检保养方案选择器：只读输入框 + 点击弹窗选择

  交互：显示为只读 el-input，点击打开弹窗（单选模式）进行选择
  Props:
    modelValue  — 绑定的点检保养方案 ID（v-model）
    disabled    — 是否禁用
  ...
  Events:
    ...
    change(item)      — 选中变化时触发，传递完整 DvCheckPlanVO（清空时为 undefined）
-->
<template>
  <div
    v-bind="attrs"
...
// ==================== 名称回显 ====================
const selectedItem = ref<DvCheckPlanVO | undefined>() // 当前选中的对象
...
const resolveItemById = async (id: number | undefined) => {
  ...
  try {
    selectedItem.value = await DvCheckPlanApi.getCheckPlan(id)
  } catch (e) {
    console.error('[DvCheckPlanSelect] resolveItemById failed:', e)
  }
}
...
/** 弹窗选中回调 */
const handleSelected = (rows: DvCheckPlanVO[]) => {
