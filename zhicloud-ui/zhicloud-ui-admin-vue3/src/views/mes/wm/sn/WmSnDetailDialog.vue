...
import { WmSnApi, WmSnVO, WmSnGroupVO } from '@/api/mes/wm/sn'
...
const detailGroup = ref<WmSnGroupVO>()
const detailList = ref<WmSnVO[]>([]) // 明细列表的数据

/** 查看 SN 码明细 */
const open = async (row: WmSnGroupVO) => {
  if (!row.uuid) {
    return
  }
...
const handleBarcode = async (row: WmSnVO) => {
  if (!row.id) {
    return
  }
