<!--
  MES 盘点方案选择器：只读输入框 + 点击弹窗选择

  交互：显示为只读 el-input，点击打开弹窗（单选模式）进行选择
  Props:
    modelValue  — 绑定的盘点方案 ID（v-model）
    disabled    — 是否禁用
    clearable   — 是否允许清空（鼠标悬停时显示清除图标）
    placeholder — 占位文字
  Events:
    update:modelValue — v-model 更新
    change(item)      — 选中盘点方案变化时触发，传递完整 StockTakingPlanVO（清空时为 undefined）
-->
<template>
  <div
    v-bind="attrs"
...
// ==================== 名称回显 ====================
const selectedItem = ref<StockTakingPlanVO | undefined>() // 当前选中的盘点方案对象
