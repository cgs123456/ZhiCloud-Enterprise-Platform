...
const list = ref<WmReturnSalesDetailVO[]>([]) // 明细列表

/** 查询明细列表 */
const getList = async () => {
  loading.value = true
  try {
    list.value = await WmReturnSalesDetailApi.getReturnSalesDetailListByLineId(props.lineId)
  } finally {
    loading.value = false
  }
}
...
const handleDelete = async (detailId: number) => {
  try {
    ...
    await WmReturnSalesDetailApi.deleteReturnSalesDetail(detailId)
