...
import { WmProductRecptApi as WmProductReceiptApi, WmProductRecptVO as WmProductReceiptVO } from '@/api/mes/wm/productreceipt'
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await WmProductReceiptApi.getProductRecpt(id)
    } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'create') {
      const res = await WmProductReceiptApi.createProductRecpt(data)
      ...
      await WmProductReceiptApi.updateProductRecpt(data)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 2. 提交入库单
    await WmProductReceiptApi.submitProductRecpt(formData.value.id!)
...
const handleStock = async () => {
  try {
    ...
    formLoading.value = true
    // 执行上架
    await WmProductReceiptApi.stockProductRecpt(formData.value.id!)
...
const handleFinish = async () => {
  try {
    ...
    formLoading.value = true
    await WmProductReceiptApi.executeProductRecpt(formData.value.id!)
