<!--
  MES 生产工单选择器：只读输入框 + 点击弹窗选择

  交互：显示为只读 el-input，点击打开弹窗（单选模式）进行选择
  Props:
    modelValue  — 绑定的工单 ID（v-model）
    disabled    — 是否禁用
    clearable   — 是否允许清空（鼠标悬停时显示清除图标）
    placeholder — 占位文字
  Events:
    update:modelValue — v-model 更新
    change(item)      — 选中工单变化时触发，传递完整 ProWorkOrderVO（清空时为 undefined）
-->
<template>
  <div
    v-bind="attrs"
...
// ==================== 名称回显 ====================
const selectedItem = ref<ProWorkOrderVO | undefined>() // 当前选中的工单对象
...
const resolveItemById = async (id: number | undefined) => {
  ...
  try {
    selectedItem.value = await ProWorkOrderApi.getWorkOrder(id)
  } catch (e) {
    console.error('[ProWorkOrderSelect] resolveItemById failed:', e)
  }
}
...
/** 弹窗选中回调 */
const handleSelected = (rows: ProWorkOrderVO[]) => {
