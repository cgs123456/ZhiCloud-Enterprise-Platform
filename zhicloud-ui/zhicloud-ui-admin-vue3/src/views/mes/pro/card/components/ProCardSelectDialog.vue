...
defineOptions({ name: 'ProCardSelectDialog' })
...
const selectedRows = ref<ProCardVO[]>([]) // 多选模式：选中行
const selectedRadioId = ref<number>() // 单选模式：选中 ID
const currentRadioRow = ref<ProCardVO>() // 单选模式：选中行对象
const preSelectedIds = ref<number[]>([]) // 打开弹窗时传入的已选 ID

/** 多选：checkbox 变化 */
const handleSelectionChange = (rows: ProCardVO[]) => {
  if (props.multiple) {
...
/** 单选：radio 变化 */
const handleRadioChange = (row: ProCardVO) => {
  currentRadioRow.value = row
}

/** 单击行：单选模式下点击整行即选中（降低操作成本），多选不处理（避免和 dblclick 冲突） */
const handleRowClick = (row: ProCardVO) => {
  if (props.multiple) {
    return
  }
...
const handleRowDblClick = (row: ProCardVO) => {
  if (props.multiple) {
