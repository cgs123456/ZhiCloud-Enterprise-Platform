<!--
  ...
  Props:
    modelValue  — 绑定的供应商 ID（v-model）
    disabled    — 是否禁用
    clearable   — 是否允许清空（鼠标悬停时显示清除图标）
    placeholder — 占位文字
  Events:
    update:modelValue — v-model 更新
    change(item)      — 选中供应商变化时触发，传递完整 MdVendorVO（清空时为 undefined）
-->
<template>
  <div
    v-bind="attrs"
  ...
  <MdVendorSelectDialog ref="dialogRef" :multiple="false" @selected="handleSelected" />
</template>
...
// ==================== 名称回显 ====================
const selectedItem = ref<MdVendorVO | undefined>() // 当前选中的供应商对象

/** 输入框显示文本：只展示供应商名称，保持简洁 */
...
const resolveItemById = async (id: number | undefined) => {
  ...
  try {
    selectedItem.value = await MdVendorApi.getVendor(id)
  } catch (e) {
    console.error('[MdVendorSelect] resolveItemById failed:', e)
  }
}
...
/** 弹窗选中回调 */
const handleSelected = (rows: MdVendorVO[]) => {
