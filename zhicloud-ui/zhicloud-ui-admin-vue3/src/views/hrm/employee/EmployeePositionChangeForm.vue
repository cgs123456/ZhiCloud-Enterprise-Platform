  ...
  <Dialog v-model="dialogVisible" :title="title" width="760px">
    ...
    >
      ...
      <el-row :gutter="18">
        ...
        <el-col :span="12">
          <el-form-item label="新岗位" prop="newPostName">
            <el-input v-model="formData.newPostName" maxlength="255" placeholder="请输入新岗位" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="18">
        <el-col :span="12">
          <el-form-item label="新职级" prop="newPostLevel">
            <el-input v-model="formData.newPostLevel" maxlength="255" placeholder="请输入新职级" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="新直属上级" prop="newLeaderEmployeeId">
            <HrmEmployeeSelect
      ...
      <el-form-item label="新工作地点" prop="newWorkAddress">
        <el-input
...
import type { HrmEmployeeTransferReqVO, HrmEmployeeVO } from '@/api/hrm/employee'
...
const props = defineProps<{
  title: string
  changeType:
    | typeof HrmEmployeeChangeType.TRANSFER
    | typeof HrmEmployeeChangeType.PROMOTION
    | typeof HrmEmployeeChangeType.DEMOTION
  submitRequest: (data: HrmEmployeeTransferReqVO) => Promise<unknown>
}>()
const message = useMessage() // 消息弹窗

const dialogVisible = ref(false) // 弹窗的是否展示
const formLoading = ref(false) // 表单的提交中
const formData = ref<HrmEmployeeTransferReqVO>({}) // 表单数据
...
const changeReasonOptions = computed(() =>
  HrmEmployeeChangeReasonOptions.filter((item) =>
    props.changeType === HrmEmployeeChangeType.DEMOTION
...
async function submitForm() {
  ...
  try {
    await props.submitRequest(formData.value)
...
function resetForm(row: HrmEmployeeVO) {
  formData.value = {
    ...
    reason:
      props.changeType === HrmEmployeeChangeType.DEMOTION
        ? HrmEmployeeChangeReason.VIOLATION
        : HrmEmployeeChangeReason.ORGANIZATION_ADJUSTMENT,
