...
const loginData = reactive({
  ...
  loginForm: {
    tenantName: '智云',
    // 安全加固：默认账密禁止硬编码 —— 会随前端产物一起发布到公网，等同公开管理员口令。
    // 改为环境变量注入，未配置时留空；本地开发需预填时，可在 .env.local 放开这两个变量。
  ...
  try {
    ...
    if (loginDataLoginForm.rememberMe) {
      ...
      authUtil.setLoginForm({
        tenantName: loginDataLoginForm.tenantName,
        username: loginDataLoginForm.username,
        rememberMe: loginDataLoginForm.rememberMe
      })
    } else {
