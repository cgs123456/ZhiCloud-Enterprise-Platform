<template>
  ...
  >
    <el-row class="mx-[-10px]">
      ...
      <el-col :span="24" class="px-10px">
        <template v-if="passwordStrength !== 'weak'">
          <p class="text-xs mt-2" :class="['font-medium', passwordStrength === 'medium' ? 'text-amber-600' : 'text-emerald-600']">
...
const passwordStrength = ref('weak') // 密码强度: weak/medium/strong
...
const updatePasswordStrength = (password: string) => {
  ...
  if (password.length >= 12 && /[0-9]/.test(password) && /[a-zA-Z]/.test(password)) strength = 'strong'
