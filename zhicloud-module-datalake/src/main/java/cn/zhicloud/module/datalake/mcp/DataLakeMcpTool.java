package cn.zhicloud.module.datalake.mcp;
...
import cn.zhicloud.module.datalake.service.DataArchivalService;
import cn.zhicloud.module.datalake.service.IcebergCatalogService;
...
public class DataLakeMcpTool {
     ...
     */
    /**
     * 合法标识符段：字母或下划线开头，仅含字母、数字、下划线
     */
    private static final Pattern IDENTIFIER_SEGMENT_PATTERN = Pattern.compile("^[A-Za-z_][A-Za-z0-9_]*$");

    /**
     * 点分标识符引用：允许 schema.table（2 段）与 catalog.schema.table（3 段），与工具示例保持一致
     */
    private static final Pattern QUALIFIED_NAME_PATTERN = Pattern.compile("[A-Za-z0-9_]+(?:\\.[A-Za-z0-9_]+)+");
    ...
    private boolean validateSelectSql(String sql) {
        ...
        // 5. 表名引用校验：允许 schema.table（2 段）与 catalog.schema.table（3 段），与工具示例一致；
        //    每一段必须是合法标识符，出现非法字符则拒绝
        if (!validateQualifiedRefs(trimmed)) {
            return false;
        }
        return true;
    }
    ...
    /**
     * 校验 SQL 中出现的点分表引用：每段必须为合法标识符（字母/下划线开头）。
     *
     * @return true 表示全部合法；false 表示包含非法标识符
     */
    private boolean validateQualifiedRefs(String sql) {
        if (sql.contains("..")) {
            log.warn("[validateSelectSql][SQL 包含非法的连续点号，拒绝执行: {}]", truncateForLog(sql));
            return false;
        }
        Matcher matcher = QUALIFIED_NAME_PATTERN.matcher(sql);
        while (matcher.find()) {
            for (String segment : matcher.group().split("\\.")) {
                if (!IDENTIFIER_SEGMENT_PATTERN.matcher(segment).matches()) {
                    log.warn("[validateSelectSql][SQL 包含非法标识符 {}，拒绝执行: {}]",
                            matcher.group(), truncateForLog(sql));
                    return false;
                }
            }
        }
