...
import cn.iocoder.yudao.module.ai.dal.dataobject.nl2sql.AiNl2SqlQueryHistoryDO;
import cn.iocoder.yudao.module.ai.dal.mysql.nl2sql.AiNl2SqlQueryHistoryMapper;
...
import static cn.iocoder.yudao.module.ai.enums.ErrorCodeConstants.NL2SQL_LLM_UNAVAILABLE;
import static cn.iocoder.yudao.module.ai.enums.ErrorCodeConstants.NL2SQL_SAFETY_CHECK_FAILED;
import static cn.iocoder.yudao.module.ai.enums.ErrorCodeConstants.NL2SQL_EXECUTE_FAILED;
...
public class Nl2SqlServiceImpl implements Nl2SqlService {
    ...
    public Nl2SqlQueryRespVO executeSql(String sql) {
        ...
        // 1. 安全校验（四层校验）
        if (!sqlSafetyValidator.validate(sql)) {
            respVO.setStatus(1).setErrorMsg("SQL 安全校验失败：仅允许只读 SELECT 查询");
            throw exception(NL2SQL_SAFETY_CHECK_FAILED);
        }
