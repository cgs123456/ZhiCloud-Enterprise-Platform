 ...
 * <h3>启用条件</h3>
 * <ul>
 *   <li>{@code zhicloud.airag.enabled=true}（AiragConfiguration 加载 VectorStore Bean）</li>
 *   <li>{@code zhicloud.airag.advisor.enabled=true}（本开关）</li>
 ...
 * 流式场景下在 nextStream 前同步执行检索并改写请求，与同步路径复用
 * {@link #enhanceWithRetrieval}；检索异常时降级放行，保证流式链路不被阻断。
 *
 * @author zhicloud
 */
@Component
@ConditionalOnProperty(prefix = "zhicloud.airag.advisor", name = "enabled", havingValue = "true")
...
public class RagAdvisor implements CallAdvisor, StreamAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request, CallAdvisorChain chain) {
        // 同步调用：先执行 RAG 检索增强（失败内部降级放行），再走下游链路
        return chain.nextCall(enhanceWithRetrieval(request));
    }

    /**
     * 流式调用入口（StreamAdvisor 实现）
     *
     * <p>在 {@code nextStream} 前同步完成 RAG 检索并改写请求，与同步路径
     * {@link #adviseCall} 复用同一检索逻辑 {@link #enhanceWithRetrieval}：
     * 无 knowledgeId / 检索失败 / 检索为空时降级放行原始请求，不阻断流式响应。
     *
    ...
    public Flux<ChatClientResponse> adviseStream(ChatClientRequest request, StreamAdvisorChain chain) {
        return chain.nextStream(enhanceWithRetrieval(request));
    }

    /**
     * 同步执行 RAG 检索并改写请求（{@link #adviseCall} 与 {@link #adviseStream} 公共逻辑）
     *
     * <p>无需增强（无 knowledgeId / 无用户问题）或检索失败 / 检索为空时，
     * 降级返回原始请求，保证业务不被 RAG 检索阻断。
     *
     * @param request 原始 ChatClient 请求
     * @return 检索增强后的请求；无需增强时返回原请求
     */
    private ChatClientRequest enhanceWithRetrieval(ChatClientRequest request) {
        ...
        try {
            ...
            log.warn("[enhanceWithRetrieval][向量检索失败，降级为无上下文 knowledgeId={}, err={}]",
                    knowledgeId, ex.toString());
            return request;
        }

        if (documents == null || documents.isEmpty()) {
            log.info("[enhanceWithRetrieval][未检索到相关文档 knowledgeId={}, questionHead={}]",
        ...
        log.info("[enhanceWithRetrieval][RAG 增强成功 knowledgeId={}, hitDocs={}, questionHead={}]",
    ...
    private List<Document> retrieveDocuments(Long knowledgeId, String question) {
        Long tenantId = cn.zhicloud.framework.tenant.core.context.TenantContextHolder.getRequiredTenantId();
