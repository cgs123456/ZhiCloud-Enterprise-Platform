...
import cn.zhicloud.module.airag.service.RerankerServiceImpl;
 ...
 * AI RAG 模块自动配置
 *
 * 通过 {@code zhicloud.airag.enabled=true} 控制向量存储相关 Bean 的加载。
 *
...
public class AiragConfiguration {
     ...
     * {@code classpath:airag/{model-path}/tokenizer.json}。可通过 {@code zhicloud.airag.embedding.model-path} 覆盖。
     */
    @Value("${zhicloud.airag.embedding.model-path:bge-base-zh-v1.5}")
    private String embeddingModelPath;

    /**
     * 创建本地 EmbeddingModel Bean（BGE-base-zh，基于 ONNX Runtime）
     *
     * <p>仅当容器中不存在其他 {@link EmbeddingModel} 时才创建，避免与 zhicloud-module-ai 的
     ...
     * {@code zhicloud-server/src/main/resources/airag/{model-path}/} 目录下。
     *
     ...
     *
     * 当 {@code zhicloud.airag.enabled=true} 且容器中存在 {@link EmbeddingModel} 时才创建。
     ...
     *
     * <p>仅当 {@code zhicloud.airag.enabled=true}（向量底表可用）时创建，
     ...
     *
     * <p>当 {@code zhicloud.airag.reranker.enabled=true} 时启用。
     * 优先加载 BGE-reranker-base（ONNX Cross-Encoder），模型不可用时 fallback 到 SimpleReranker。
     *
     * @return RerankerService 实例
     */
    @Bean
    @ConditionalOnProperty(prefix = "zhicloud.airag.reranker", name = "enabled", havingValue = "true")
