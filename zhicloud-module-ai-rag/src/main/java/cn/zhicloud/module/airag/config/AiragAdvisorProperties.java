 ...
 * AI RAG Advisor 配置类
 *
 * <p>对应 zhicloud-module-ai-rag 的 Advisor 链集成（P5：RAG Advisor 集成）。
 * 通过 {@code zhicloud.airag.advisor.*} 控制 RagAdvisor 的行为。
 *
 * <h3>配置项</h3>
 * <ul>
 ...
 */
@Configuration
@ConfigurationProperties(prefix = "zhicloud.airag.advisor")
@Data
public class AiragAdvisorProperties {
     ...
     * 是否启用 RagAdvisor
     *
     * <p>启用条件：{@code zhicloud.airag.enabled=true}（VectorStore Bean 存在） + 本开关为 true。
     * 默认关闭，避免无向量库环境下加载失败。
     */
