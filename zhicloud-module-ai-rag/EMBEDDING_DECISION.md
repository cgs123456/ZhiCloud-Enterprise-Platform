# zhicloud-module-ai-rag Embedding 模型选型决策

> 本文档记录 zhicloud-module-ai-rag 模块在 Embedding 模型选型上的对比与结论。
...
- zhicloud 为单体/单体模块化架构，减少独立服务运维成本；
...
```java
// 需引入 spring-ai-transformers 依赖（如未在 zhicloud-module-ai 中引入，需在本模块 pom.xml 补充）
@Bean
...
#### 方式 B：复用 zhicloud-module-ai 的 AiModelService

在 zhicloud 后台「AI 模型管理」中配置一个 EMBEDDING 类型的模型（平台选 OpenAI 兼容 / Ollama 等），由 `AiModelService.getOrCreateVectorStore(modelId, ...)` 创建 VectorStore。此方式依赖外部 Embedding 服务，**不符合完全本地化**诉求，仅作为 fallback。

### 4.3 当前实现状态

由于 PostgreSQL + pgvector 当前未部署，本模块的 `AiragConfiguration` 通过 `@ConditionalOnProperty(prefix = "zhicloud.airag", name = "enabled", havingValue = "true")` 控制 `PgVectorStore` Bean 的加载，且 `PgVectorStore` 依赖容器中存在 `EmbeddingModel` Bean 才创建。
...
3. 配置 `zhicloud.airag.enabled=true`；
