...
--      用于批量拣货、批量复核、批量出库。
-- 注意：Flyway 迁移脚本必须可重入（重复执行不报错），因此：
--   1. 使用 CREATE TABLE IF NOT EXISTS，禁止 DROP TABLE（避免破坏存量数据）
--   2. INSERT 使用 INSERT IGNORE 或 WHERE NOT EXISTS 防止唯一键冲突
--   3. system_menu / system_dict_type / system_dict_data 无 tenant_id 列，不可引用
...
-- ----------------------------
-- 权限初始化（system_menu 无 tenant_id 列，使用 permission 作为唯一标识避免重复）
-- ----------------------------
...
VALUES
('波次单管理', '', 2, 30, 0, '/wms/wave-order', '', 'wms/waveOrder/index', 0, 'admin', NOW(), 'admin', NOW(), 0),
('波次单查询', 'wms:wave-order:query', 3, 1, 0, '', '', '', 0, 'admin', NOW(), 'admin', NOW(), 0),
('波次单创建', 'wms:wave-order:create', 3, 2, 0, '', '', '', 0, 'admin', NOW(), 'admin', NOW(), 0),
('波次单更新', 'wms:wave-order:update', 3, 3, 0, '', '', '', 0, 'admin', NOW(), 'admin', NOW(), 0),
('波次单删除', 'wms:wave-order:delete', 3, 4, 0, '', '', '', 0, 'admin', NOW(), 'admin', NOW(), 0);

-- ----------------------------
-- 字典初始化（system_dict_type / system_dict_data 无 tenant_id 列）
-- ----------------------------
INSERT IGNORE INTO system_dict_type (name, type, status, creator, create_time, updater, update_time, deleted, remark)
VALUES ('WMS 波次策略', 'wms_wave_strategy', 0, 'admin', NOW(), 'admin', NOW(), 0, 'WMS 波次策略字典');
...
VALUES
(1, '按仓库合并', '1', 'wms_wave_strategy', 0, 'primary', '', '', 'admin', NOW(), 'admin', NOW(), 0),
(2, '按客户合并', '2', 'wms_wave_strategy', 0, 'success', '', '', 'admin', NOW(), 'admin', NOW(), 0),
(3, '按商品合并', '3', 'wms_wave_strategy', 0, 'info', '', '', 'admin', NOW(), 'admin', NOW(), 0),
(4, '按承运商合并', '4', 'wms_wave_strategy', 0, 'warning', '', '', 'admin', NOW(), 'admin', NOW(), 0);
