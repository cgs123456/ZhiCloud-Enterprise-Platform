 ...
 *
 * <p>问题：yudao-server 启动类在 {@code cn.iocoder.yudao.server}，
 * 但业务模块在 {@code cn.iocoder.yudao.module.*}（兄弟包，非子包）。
 * Modulith 默认以 {@link ApplicationRuntime#getMainApplicationClass()} 的包为根检测模块，
 * 因此扫描 {@code cn.iocoder.yudao.server.*}，找不到业务模块的 {@code @ApplicationModule} 注解。
 *
 * <p>修复：
 * <ol>
 *   <li>显式构建 {@link ApplicationModules}，指定根包为 {@code cn.iocoder.yudao.module}，
...
public class ModulithConfiguration {
    ...
    /**
     * 业务模块根包：所有 yudao-module-* 模块的公共父包
     */
    private static final String MODULE_BASE_PACKAGE = "cn.iocoder.yudao.module";

    /**
     * 自定义 {@link ApplicationModules} Bean，从业务模块根包检测。
     *
     * <p>覆盖 Modulith 默认的 {@code ModulithAutoConfiguration.applicationModules(...)}，
     * 后者使用启动类所在包 {@code cn.iocoder.yudao.server}，扫描不到业务模块。
     */
    @Bean
