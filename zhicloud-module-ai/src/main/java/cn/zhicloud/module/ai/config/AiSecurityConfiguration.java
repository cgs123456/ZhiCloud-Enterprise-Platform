package cn.zhicloud.module.ai.config;
 ...
 *
 * 读取 {@code zhicloud.ai.security.*} 配置，覆盖四道防线：
 ...
 *
 * 配置示例见 zhicloud-server/src/main/resources/application.yaml 中 {@code zhicloud.ai.security} 注释段。
 * 所有配置项均提供默认值，未配置时不改变现有行为（默认关闭或仅告警）。
 *
 * @author 智云
 */
@Configuration("zhicloudAiSecurityProps") // 显式命名 zhicloudAiSecurityProps，避免与 framework.security.config.SecurityConfiguration 的 bean 名 aiSecurityConfiguration 冲突
@ConfigurationProperties(prefix = "zhicloud.ai.security")
@Data
