package cn.zhicloud.module.ai.util;
...
import static cn.zhicloud.module.ai.enums.ErrorCodeConstants.API_CONFIG_PLACEHOLDER_NOT_RESOLVED;
...
public class AiUtils {
    ...
    public static Message buildMessage(String type, String content) {
        ...
        if (MessageType.TOOL.getValue().equals(type)) {
            // 降级处理：调用方仅传入文本内容，缺少 toolCallId 等元数据，无法精确还原 ToolResponseMessage。
            // 这里将 content 包装为 ToolResponseMessage 返回，避免抛出 UnsupportedOperationException 中断 AI 对话。
            // 使用占位的 id 与 name，使消息可被 Spring AI 正常消费；如需精确匹配 toolCallId，应由调用方直接构造 ToolResponseMessage。
            return ToolResponseMessage.builder()
                    .responses(List.of(
                            new ToolResponseMessage.ToolResponse("default", "tool", content)))
                    .build();
        }
