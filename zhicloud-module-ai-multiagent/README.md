# zhicloud-module-ai-multiagent

> 基于 Spring AI 1.1.8 的多 Agent 编排模块：实现 Supervisor-Worker 架构与 ReAct（Reasoning + Acting）循环。

## 1. 模块简介

`zhicloud-module-ai-multiagent` 是 zhicloud 项目的可选模块（Spring Modulith 声明 A3），在 `zhicloud-module-ai` 之上提供**多 Agent 编排能力**：
...
- `List<ToolCallback>`（来自 `zhicloud-module-ai` 的 `AiAutoConfiguration` 注册的 MCP / 业务工具）
...
| Spring Boot | 3.5（继承自 zhicloud 父项目） |
| JDK | 21（继承自 zhicloud 父项目，支持虚拟线程） |
| ChatClient | Spring AI 1.1.8 `ChatClient.builder(chatModel).build()` |
| MyBatis Plus | 继承自 `zhicloud-spring-boot-starter-mybatis` |
| 多租户 | 继承自 `zhicloud-spring-boot-starter-biz-tenant` |

## 4. 模块结构

```
zhicloud-module-ai-multiagent/
├── pom.xml                                # 依赖 zhicloud-module-ai（optional）+ spring-ai-bom 1.1.8
└── src/main/java/cn/zhicloud/zhicloud/module/aimultiagent/
    ├── AiMultiAgentApplication.java       # 入口标记类（由 zhicloud-server 扫描加载）
...
所有控制器位于 `controller.admin` 包下，自动继承 zhicloud 框架的 `/admin-api` 前缀。
...
### 6.1 与 zhicloud-module-ai 的关系

本模块 `pom.xml` 中 `zhicloud-module-ai` 声明为 `<optional>true</optional>` 依赖：
...
- **共享 `ToolCallback`**：自动收集 zhicloud-module-ai 的 `AiAutoConfiguration` 注册的 MCP / 业务工具
- **不反向依赖**：zhicloud-module-ai 不依赖本模块，可独立部署
...
private ChatClient chatClient;            // zhicloud-module-ai 未直接提供时为 null
...
@Autowired(required = false)
private List<ToolCallback> toolCallbacks;  // zhicloud-module-ai AiAutoConfiguration 注册
```
...
- **必须配置 LLM API Key**：在 zhicloud 后台「AI 模型管理」中配置至少一个 `CHAT` 类型模型，并设为默认（供 `AiModelService.getRequiredDefaultModel(CHAT)` 获取）
- **可选：MCP 工具**：如需 ReAct Agent 调用业务工具，需在 zhicloud-module-ai 中通过 `@Tool` 注解或 MCP Server 注册工具
