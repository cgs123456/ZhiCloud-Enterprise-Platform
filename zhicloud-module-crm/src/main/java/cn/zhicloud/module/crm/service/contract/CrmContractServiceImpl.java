...
import cn.zhicloud.module.crm.controller.admin.contract.vo.contract.CrmContractPageReqVO;
import cn.zhicloud.module.crm.controller.admin.contract.vo.contract.CrmContractSaveReqVO;
...
import cn.zhicloud.module.crm.dal.dataobject.contract.CrmContractProductDO;
import cn.zhicloud.module.crm.dal.mysql.contract.CrmContractMapper;
import cn.zhicloud.module.crm.dal.mysql.contract.CrmContractProductMapper;
...
public class CrmContractServiceImpl implements CrmContractService {
    ...
    public void updateContract(CrmContractSaveReqVO updateReqVO) {
        ...
        updateObj.setVersion(oldContract.getVersion());
        int updated = contractMapper.updateById(updateObj);
        if (updated == 0) {
    ...
    private void calculateTotalPrice(CrmContractDO contract, List<CrmContractProductDO> contractProducts) {
        ...
        contract.setTotalPrice(contract.getTotalProductPrice().subtract(discountPrice == null ? BigDecimal.ZERO : discountPrice));
    }
    ...
    public void transferContract(CrmContractTransferReqVO reqVO, Long userId) {
        ...
        // 2.2 设置负责人
        CrmContractDO updateObj = new CrmContractDO().setId(reqVO.getId()).setOwnerUserId(reqVO.getNewOwnerUserId());
        updateObj.setVersion(contract.getVersion());
        int updated = contractMapper.updateById(updateObj);
        if (updated == 0) {
            throw exception(CRM_OPTIMISTIC_LOCK_FAIL);
        }
    ...
    public void updateContractFollowUp(Long id, LocalDateTime contactNextTime, String contactLastContent) {
        ...
        // 2. 更新联系人的跟进信息
        CrmContractDO updateObj = new CrmContractDO().setId(id).setContactLastTime(LocalDateTime.now());
        updateObj.setVersion(contract.getVersion());
        int updated = contractMapper.updateById(updateObj);
        if (updated == 0) {
            throw exception(CRM_OPTIMISTIC_LOCK_FAIL);
    ...
    public void submitContract(Long id, Long userId) {
        ...
        // 3. 更新合同工作流编号
        CrmContractDO updateObj = new CrmContractDO().setId(id).setProcessInstanceId(processInstanceId)
    ...
    public void updateContractAuditStatus(Long id, Integer bpmResult) {
        ...
        CrmContractDO updateObj = new CrmContractDO().setId(id).setAuditStatus(auditStatus);
        updateObj.setVersion(contract.getVersion());
        int updated = contractMapper.updateById(updateObj);
        if (updated == 0) {
            throw exception(CRM_OPTIMISTIC_LOCK_FAIL);
