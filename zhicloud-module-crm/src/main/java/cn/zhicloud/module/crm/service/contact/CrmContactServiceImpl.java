package cn.zhicloud.module.crm.service.contact;
...
import cn.zhicloud.module.crm.controller.admin.contact.vo.CrmContactPageReqVO;
import cn.zhicloud.module.crm.controller.admin.contact.vo.CrmContactSaveReqVO;
...
import cn.zhicloud.module.crm.dal.mysql.contact.CrmContactMapper;
...
public class CrmContactServiceImpl implements CrmContactService {
    ...
    public void updateContact(CrmContactSaveReqVO updateReqVO) {
        ...
        updateObj.setVersion(oldContact.getVersion());
        int updated = contactMapper.updateById(updateObj);
        if (updated == 0) {
    ...
    public void transferContact(CrmContactTransferReqVO reqVO, Long userId) {
        ...
        // 2.2 设置新的负责人
        CrmContactDO updateObj = new CrmContactDO().setId(reqVO.getId()).setOwnerUserId(reqVO.getNewOwnerUserId());
        updateObj.setVersion(contact.getVersion());
        int updated = contactMapper.updateById(updateObj);
        if (updated == 0) {
            throw exception(CRM_OPTIMISTIC_LOCK_FAIL);
        }
    ...
    public void updateContactFollowUp(Long id, LocalDateTime contactNextTime, String contactLastContent) {
        ...
        // 2. 更新联系人的跟进信息
        CrmContactDO updateObj = new CrmContactDO().setId(id).setContactNextTime(contactNextTime)
