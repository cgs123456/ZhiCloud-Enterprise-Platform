...
import cn.zhicloud.module.crm.controller.admin.business.vo.business.CrmBusinessTransferReqVO;
import cn.zhicloud.module.crm.controller.admin.contact.vo.CrmContactTransferReqVO;
import cn.zhicloud.module.crm.controller.admin.contract.vo.contract.CrmContractTransferReqVO;
import cn.zhicloud.module.crm.controller.admin.customer.vo.customer.*;
...
import cn.zhicloud.module.crm.dal.dataobject.customer.CrmCustomerPoolConfigDO;
...
import cn.zhicloud.module.crm.enums.common.CrmSceneTypeEnum;
...
import static cn.zhicloud.framework.common.util.collection.CollectionUtils.filterList;
import static cn.zhicloud.module.crm.enums.ErrorCodeConstants.*;
import static cn.zhicloud.module.crm.enums.LogRecordConstants.*;
import static cn.zhicloud.module.crm.enums.customer.CrmCustomerLimitConfigTypeEnum.CUSTOMER_LOCK_LIMIT;
import static cn.zhicloud.module.crm.enums.customer.CrmCustomerLimitConfigTypeEnum.CUSTOMER_OWNER_LIMIT;
...
public class CrmCustomerServiceImpl implements CrmCustomerService {
    ...
    public void updateCustomer(CrmCustomerSaveReqVO updateReqVO) {
        ...
        updateObj.setVersion(oldCustomer.getVersion());
        int updated = customerMapper.updateById(updateObj);
        if (updated == 0) {
    ...
    public void updateCustomerDealStatus(Long id, Boolean dealStatus) {
        ...
        // 2. 更新客户的成交状态
        CrmCustomerDO updateObj = new CrmCustomerDO().setId(id).setDealStatus(dealStatus);
        updateObj.setVersion(customer.getVersion());
        int updated = customerMapper.updateById(updateObj);
        if (updated == 0) {
            throw exception(CRM_OPTIMISTIC_LOCK_FAIL);
        }
    ...
    public void updateCustomerFollowUp(Long id, LocalDateTime contactNextTime, String contactLastContent) {
        ...
        // 2. 更新客户的跟进信息
        CrmCustomerDO updateObj = new CrmCustomerDO().setId(id).setFollowUpStatus(true).setContactNextTime(contactNextTime)
    ...
    public void transferCustomer(CrmCustomerTransferReqVO reqVO, Long userId) {
        ...
        // 2.2 转移后重新设置负责人
        CrmCustomerDO updateObj = new CrmCustomerDO().setId(reqVO.getId())
    ...
    public void lockCustomer(CrmCustomerLockReqVO lockReqVO, Long userId) {
        ...
        // 2. 更新锁定状态
        CrmCustomerDO updateObj = BeanUtils.toBean(lockReqVO, CrmCustomerDO.class);
        updateObj.setVersion(customer.getVersion());
        int updated = customerMapper.updateById(updateObj);
        if (updated == 0) {
            throw exception(CRM_OPTIMISTIC_LOCK_FAIL);
    ...
    public CrmCustomerImportRespVO importCustomerList(List<CrmCustomerImportExcelVO> importCustomers,
        ...
        // 1. 预载同名客户：一次 IN 查询建立 Map 查重，避免循环内逐条 selectByCustomerName 的 N+1
        Map<String, CrmCustomerDO> existCustomerMap = customerMapper.selectListByNames(
                        CollectionUtils.convertSet(importCustomers, CrmCustomerImportExcelVO::getName)).stream()
                .collect(Collectors.toMap(CrmCustomerDO::getName, customer -> customer, (a, b) -> a));

        // 逐条校验并分类为新建/更新，收集后分组批量落库
        ...
        List<CrmCustomerDO> toCreateCustomers = new ArrayList<>();
        List<CrmCustomerDO> toUpdateCustomers = new ArrayList<>();
        importCustomers.forEach(importCustomer -> {
            ...
            // 情况一：判断如果不存在，在进行插入
            CrmCustomerDO existCustomer = existCustomerMap.get(importCustomer.getName());
            if (existCustomer == null) {
                toCreateCustomers.add(initCustomer(importCustomer, importReqVO.getOwnerUserId()));
            ...
            // 2.1 更新客户信息
            toUpdateCustomers.add(BeanUtils.toBean(importCustomer, CrmCustomerDO.class)
                    .setId(existCustomer.getId()));
            respVO.getUpdateCustomerNames().add(importCustomer.getName());
        });
        // 2. 批量插入新建客户；回填 ID 后创建数据权限、记录操作日志
        if (CollUtil.isNotEmpty(toCreateCustomers)) {
            customerMapper.insertBatch(toCreateCustomers);
            for (int i = 0; i < toCreateCustomers.size(); i++) {
                CrmCustomerDO customer = toCreateCustomers.get(i);
                if (importReqVO.getOwnerUserId() != null) {
                    permissionService.createPermission(new CrmPermissionCreateReqBO()
                            ...
                            .setBizId(customer.getId()).setUserId(importReqVO.getOwnerUserId())
        ...
        // 3. 批量更新已存在客户并记录操作日志
        if (CollUtil.isNotEmpty(toUpdateCustomers)) {
            customerMapper.updateBatch(toUpdateCustomers);
            toUpdateCustomers.forEach(customer -> getSelf().importCustomerLog(customer, true));
        }
        return respVO;
    }
