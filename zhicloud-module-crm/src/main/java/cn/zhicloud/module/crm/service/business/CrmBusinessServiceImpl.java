package cn.zhicloud.module.crm.service.business;
...
import cn.zhicloud.module.crm.controller.admin.business.vo.business.CrmBusinessPageReqVO;
import cn.zhicloud.module.crm.controller.admin.business.vo.business.CrmBusinessSaveReqVO;
import cn.zhicloud.module.crm.controller.admin.business.vo.business.CrmBusinessTransferReqVO;
import cn.zhicloud.module.crm.controller.admin.business.vo.business.CrmBusinessUpdateStatusReqVO;
import cn.zhicloud.module.crm.controller.admin.contact.vo.CrmContactBusinessReqVO;
import cn.zhicloud.module.crm.controller.admin.statistics.vo.funnel.CrmStatisticsFunnelReqVO;
import cn.zhicloud.module.crm.dal.dataobject.business.CrmBusinessDO;
import cn.zhicloud.module.crm.dal.dataobject.business.CrmBusinessProductDO;
import cn.zhicloud.module.crm.dal.dataobject.business.CrmBusinessStatusDO;
import cn.zhicloud.module.crm.dal.dataobject.contact.CrmContactBusinessDO;
import cn.zhicloud.module.crm.dal.dataobject.contact.CrmContactDO;
import cn.zhicloud.module.crm.dal.mysql.business.CrmBusinessMapper;
import cn.zhicloud.module.crm.dal.mysql.business.CrmBusinessProductMapper;
...
import cn.zhicloud.module.crm.service.contact.CrmContactBusinessService;
...
public class CrmBusinessServiceImpl implements CrmBusinessService {
    ...
    public Long createBusiness(CrmBusinessSaveReqVO createReqVO, Long userId) {
        ...
        if (CollUtil.isEmpty(statusList)) {
            throw exception(BUSINESS_STATUS_LIST_EMPTY);
        }
    ...
    public void updateBusiness(CrmBusinessSaveReqVO updateReqVO) {
        ...
        updateObj.setVersion(oldBusiness.getVersion());
        int updated = businessMapper.updateById(updateObj);
        if (updated == 0) {
    ...
    public void updateBusinessFollowUp(Long id, LocalDateTime contactNextTime, String contactLastContent) {
        ...
        // 2. 更新联系人的跟进信息
        CrmBusinessDO updateObj = new CrmBusinessDO().setId(id).setFollowUpStatus(true).setContactNextTime(contactNextTime)
                .setContactLastTime(LocalDateTime.now());
        updateObj.setVersion(business.getVersion());
        int updated = businessMapper.updateById(updateObj);
        if (updated == 0) {
            throw exception(CRM_OPTIMISTIC_LOCK_FAIL);
        }
    ...
    private void calculateTotalPrice(CrmBusinessDO business, List<CrmBusinessProductDO> businessProducts) {
        ...
        // discountPrice 为 null（折扣百分比未填）时视为不打折，避免 subtract(null) 抛 NPE
        business.setTotalPrice(business.getTotalProductPrice().subtract(discountPrice == null ? BigDecimal.ZERO : discountPrice));
    }
    ...
    public void updateBusinessStatus(CrmBusinessUpdateStatusReqVO reqVO) {
        ...
        // 2. 更新商机状态
        CrmBusinessDO updateObj = new CrmBusinessDO().setId(reqVO.getId()).setStatusId(reqVO.getStatusId())
