...
import cn.zhicloud.module.crm.controller.admin.contract.vo.contract.CrmContractRespVO;
...
import cn.zhicloud.module.crm.service.salesorder.CrmSaleOrderService;
...
public class CrmContractController {
    ...
    private List<CrmContractRespVO> buildContractDetailList(List<CrmContractDO> contractList) {
        ...
        // 1.6 获得已下单金额
        Map<Long, BigDecimal> saleOrderPriceMap = saleOrderService.getSaleOrderPriceMapByContractId(
                convertSet(contractList, CrmContractDO::getId));
        // 2. 拼接数据
        return BeanUtils.toBean(contractList, CrmContractRespVO.class, contractVO -> {
            ...
            // 2.6 设置已下单金额
            contractVO.setTotalSaleOrderPrice(saleOrderPriceMap.getOrDefault(contractVO.getId(), BigDecimal.ZERO));
        });
    }
    ...
    public CommonResult<List<CrmContractRespVO>> getContractSimpleList(@RequestParam("customerId") Long customerId) {
        ...
        return success(convertList(pageResult.getList(), contract -> new CrmContractRespVO() // 只返回 id、name 等精简字段
                ...
                .setTotalSaleOrderPrice(saleOrderPriceMap.getOrDefault(contract.getId(), BigDecimal.ZERO))));
    }
