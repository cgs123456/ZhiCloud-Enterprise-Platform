...
import cn.zhicloud.module.erp.controller.admin.stock.vo.out.ErpStockOutPageReqVO;
import cn.zhicloud.module.erp.controller.admin.stock.vo.out.ErpStockOutSaveReqVO;
...
import cn.zhicloud.module.erp.dal.dataobject.stock.ErpStockOutItemDO;
import cn.zhicloud.module.erp.dal.mysql.stock.ErpStockOutItemMapper;
...
import cn.zhicloud.module.erp.service.sale.ErpCustomerService;
...
public class ErpStockOutServiceImpl implements ErpStockOutService {
    ...
    public void updateStockOutStatus(Long id, Integer status) {
        ...
        stockOutItems.forEach(stockOutItem -> {
            List<Long> key = List.of(stockOutItem.getProductId(), stockOutItem.getWarehouseId());
            BigDecimal count = approve ? stockOutItem.getCount().negate() : stockOutItem.getCount();
            mergedCountMap.merge(key, count, BigDecimal::add);
            firstItemIdMap.putIfAbsent(key, stockOutItem.getId());
        });
        mergedCountMap.forEach((key, count) -> stockRecordService.createStockRecord(new ErpStockRecordCreateReqBO(
                ...
                bizType, id, firstItemIdMap.get(key), stockOut.getNo())));
    }
