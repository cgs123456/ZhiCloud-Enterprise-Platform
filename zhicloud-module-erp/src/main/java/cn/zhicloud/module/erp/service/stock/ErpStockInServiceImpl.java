...
import cn.zhicloud.module.erp.controller.admin.stock.vo.in.ErpStockInPageReqVO;
import cn.zhicloud.module.erp.controller.admin.stock.vo.in.ErpStockInSaveReqVO;
...
import cn.zhicloud.module.erp.dal.dataobject.stock.ErpStockInItemDO;
import cn.zhicloud.module.erp.dal.mysql.stock.ErpStockInItemMapper;
...
public class ErpStockInServiceImpl implements ErpStockInService {
    ...
    public void updateStockInStatus(Long id, Integer status) {
        ...
        stockInItems.forEach(stockInItem -> {
            List<Long> key = List.of(stockInItem.getProductId(), stockInItem.getWarehouseId());
            BigDecimal count = approve ? stockInItem.getCount() : stockInItem.getCount().negate();
            mergedCountMap.merge(key, count, BigDecimal::add);
            firstItemIdMap.putIfAbsent(key, stockInItem.getId());
        });
        mergedCountMap.forEach((key, count) -> stockRecordService.createStockRecord(new ErpStockRecordCreateReqBO(
                ...
                bizType, id, firstItemIdMap.get(key), stockIn.getNo())));
    }
