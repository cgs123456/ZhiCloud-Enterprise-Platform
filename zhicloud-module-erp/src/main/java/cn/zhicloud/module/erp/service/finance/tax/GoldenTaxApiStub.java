 ...
 *
 * <p>开关：{@code zhicloud.erp.golden-tax.enabled}，默认关闭（false）。
 * 仅在显式设置为 true 时，本占位实现才会被加载（便于开发/演示）。
 *
 * @deprecated 金税四期接口占位实现，生产环境必须替换为真实 SDK。请在 {@code zhicloud.erp.golden-tax.enabled=true}
 * 配合真实证书后，替换为接入金税 SDK 的实现类。
 */
@Deprecated
@Slf4j
@Component
@ConditionalOnProperty(name = "zhicloud.erp.golden-tax.enabled", havingValue = "true")
