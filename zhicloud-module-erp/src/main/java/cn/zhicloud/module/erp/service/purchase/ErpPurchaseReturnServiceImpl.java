package cn.zhicloud.module.erp.service.purchase;
...
import cn.zhicloud.module.erp.controller.admin.purchase.vo.returns.ErpPurchaseReturnPageReqVO;
import cn.zhicloud.module.erp.controller.admin.purchase.vo.returns.ErpPurchaseReturnSaveReqVO;
import cn.zhicloud.module.erp.dal.dataobject.product.ErpProductDO;
import cn.zhicloud.module.erp.dal.dataobject.purchase.ErpPurchaseOrderDO;
import cn.zhicloud.module.erp.dal.dataobject.purchase.ErpPurchaseReturnDO;
import cn.zhicloud.module.erp.dal.dataobject.purchase.ErpPurchaseReturnItemDO;
import cn.zhicloud.module.erp.dal.mysql.purchase.ErpPurchaseReturnItemMapper;
import cn.zhicloud.module.erp.dal.mysql.purchase.ErpPurchaseReturnMapper;
import cn.zhicloud.module.erp.dal.redis.no.ErpNoRedisDAO;
import cn.zhicloud.module.erp.enums.ErpAuditStatus;
import cn.zhicloud.module.erp.enums.stock.ErpStockRecordBizTypeEnum;
...
import cn.zhicloud.module.erp.service.stock.ErpStockRecordService;
import cn.zhicloud.module.erp.service.stock.bo.ErpStockRecordCreateReqBO;
...
public class ErpPurchaseReturnServiceImpl implements ErpPurchaseReturnService {
    ...
    public void updatePurchaseReturnStatus(Long id, Integer status) {
        ...
        // 3. 变更库存：按 (productId, warehouseId) 归并数量后逐组创建，减少库存更新往返
        ...
        Map<List<Long>, BigDecimal> mergedCountMap = new LinkedHashMap<>();
        Map<List<Long>, Long> firstItemIdMap = new LinkedHashMap<>();
        purchaseReturnItems.forEach(purchaseReturnItem -> {
            List<Long> key = List.of(purchaseReturnItem.getProductId(), purchaseReturnItem.getWarehouseId());
            BigDecimal count = approve ? purchaseReturnItem.getCount().negate() : purchaseReturnItem.getCount();
            mergedCountMap.merge(key, count, BigDecimal::add);
            firstItemIdMap.putIfAbsent(key, purchaseReturnItem.getId());
        });
        mergedCountMap.forEach((key, count) -> stockRecordService.createStockRecord(new ErpStockRecordCreateReqBO(
                key.get(0), key.get(1), count,
                bizType, id, firstItemIdMap.get(key), purchaseReturn.getNo())));
    }
