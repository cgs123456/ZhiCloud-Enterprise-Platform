...
import cn.zhicloud.module.erp.dal.mysql.finance.ErpFixedAssetDepreciationMapper;
import cn.zhicloud.module.erp.dal.mysql.finance.ErpFixedAssetMapper;
...
public class ErpFixedAssetServiceImpl implements ErpFixedAssetService {
    ...
    public BatchDepreciationResult batchCalculateMonthlyDepreciation(List<Long> assetIds, LocalDate period) {
        ...
        // 2. 逐个资产计提折旧：经 getSelf() 代理调用，保证单资产事务生效；单个失败不影响其他资产
        List<String> failureList = new ArrayList<>();
        for (Long assetId : assetIds) {
            try {
                Long depId = getSelf().calculateMonthlyDepreciation(assetId, periodDO.getId());
                if (depId != null) {
                ...
                String assetCode = String.valueOf(assetId);
                ErpFixedAssetDO asset = fixedAssetMapper.selectById(assetId);
                if (asset != null) {
                    assetCode = asset.getCode();
                }
                failureList.add(assetCode + "(" + e.getMessage() + ")");
            }
        }
        log.info("[batchCalculateMonthlyDepreciation][期间 {} 批量计提完成，成功 {} 个，总金额 {}]",
                periodCode, successCount, totalAmount);
        // 3. 存在失败项时抛出汇总异常，让调用方明确感知
        if (!failureList.isEmpty()) {
            throw exception(FIXED_ASSET_BATCH_DEPRECIATION_PARTIAL_FAIL, String.join(", ", failureList));
        }
     ...
     * @return 自己
     */
    private ErpFixedAssetServiceImpl getSelf() {
