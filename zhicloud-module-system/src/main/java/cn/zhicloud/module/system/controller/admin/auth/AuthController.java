package cn.zhicloud.module.system.controller.admin.auth;
...
import cn.zhicloud.framework.ratelimiter.core.keyresolver.impl.ClientIpRateLimiterKeyResolver;
import cn.zhicloud.framework.ratelimiter.core.keyresolver.impl.ExpressionRateLimiterKeyResolver;
...
import cn.zhicloud.module.system.service.auth.AdminAuthService;
...
import cn.zhicloud.module.system.service.social.SocialClientService;
...
public class AuthController {
    ...
    // P0-4 修复：启用 IP 限流，60 秒内最多 10 次登录尝试，防止暴力撞库
    @RateLimiter(time = 60, count = 10, keyResolver = ClientIpRateLimiterKeyResolver.class)
    ...
    @PermitAll
    @Operation(summary = "使用 TOTP 双因素认证登录", description = "账号密码验证通过后，需要 TOTP 验证码完成登录")
    public CommonResult<AuthLoginRespVO> loginByTotp(@RequestBody @Valid AuthLoginByTotpReqVO reqVO) {
        return success(authService.loginByTotp(reqVO));
    }
    ...
    // P0-4 修复：启用 IP 限流，60 秒内最多 5 次注册，防止批量注册
    ...
    // P0-4 修复：启用限流，按手机号 60 秒内最多 1 条短信验证码
    @RateLimiter(time = 60, count = 1, keyResolver = ExpressionRateLimiterKeyResolver.class, keyArg = "#reqVO.mobile")
