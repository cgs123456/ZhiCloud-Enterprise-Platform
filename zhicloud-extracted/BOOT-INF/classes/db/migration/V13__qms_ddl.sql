...
CREATE TABLE IF NOT EXISTS qms_fmea_item (
    ...
    -- 注意：`function` 是 MySQL 8.0 保留字，裸用会 ERROR 1064；
    -- 且 MyBatis-Plus 默认不转义标识符，故落库列名为 item_function，
    -- Java 侧通过 @TableField("item_function") 映射，对外 API 字段名仍为 function。
    item_function VARCHAR(500) COMMENT '功能',
