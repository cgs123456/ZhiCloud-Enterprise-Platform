...
import cn.zhicloud.module.wms.controller.admin.abc.vo.WmsAbcReportItemRespVO;
...
import cn.zhicloud.module.wms.dal.mysql.md.item.WmsItemMapper;
...
public class WmsAbcAnalysisServiceImpl implements WmsAbcAnalysisService {
    ...
    public WmsAbcReportRespVO analyzeAbcClassification(WmsAbcAnalysisReqVO reqVO) {
        // 1. 按 SKU 聚合出库频次、出库量、出库金额（GROUP BY 下推 SQL，避免全量拉取出库流水）
        Map<Long, SkuAggregate> skuAggregateMap = aggregateBySku(
    ...
    private Map<Long, SkuAggregate> aggregateBySku(Integer orderType, LocalDateTime startTime, LocalDateTime endTime) {
        Map<Long, SkuAggregate> map = new HashMap<>();
        // GROUP BY 下推 SQL：一次聚合查询代替全量流水拉取 + 内存逐行累加
        List<WmsInventoryHistoryMapper.SkuOutboundSummary> summaries =
                inventoryHistoryMapper.selectOutboundSummaryGroupBySku(orderType, startTime, endTime);
        if (CollUtil.isEmpty(summaries)) {
            return map;
        }
        for (WmsInventoryHistoryMapper.SkuOutboundSummary summary : summaries) {
            SkuAggregate agg = new SkuAggregate();
            agg.outCount = summary.getOutCount() == null ? 0L : summary.getOutCount();
            agg.outQuantity = summary.getOutQuantity() == null ? BigDecimal.ZERO : summary.getOutQuantity();
            agg.outAmount = summary.getOutAmount() == null ? BigDecimal.ZERO : summary.getOutAmount();
            map.put(summary.getSkuId(), agg);
        }
        return map;
    }
    ...
    private void resetUnclassifiedItemsAbc(Set<Long> classifiedItemIds) {
        // 下推 SQL：未出现在本轮出库流水中的商品直接置为 C 类（ids 为空时全表更新为 C）
        itemMapper.updateAbcClassificationToCExcluding(classifiedItemIds);
    }
