package cn.zhicloud.module.wms.controller.admin.order.shipment;
...
import cn.zhicloud.module.wms.controller.admin.order.shipment.vo.detail.WmsShipmentOrderDetailRespVO;
import cn.zhicloud.module.wms.controller.admin.order.shipment.vo.order.WmsShipmentOrderPageReqVO;
import cn.zhicloud.module.wms.controller.admin.order.shipment.vo.order.WmsShipmentOrderRespVO;
import cn.zhicloud.module.wms.controller.admin.order.shipment.vo.order.WmsShipmentOrderSaveReqVO;
...
public class WmsShipmentOrderController {
    ...
    @PutMapping("/complete")
    @Operation(summary = "完成出库（兼容接口：草稿直接完成）")
    @Parameter(name = "id", description = "编号", required = true)
    ...
    @PutMapping("/start-pick")
    @Operation(summary = "开始拣货（草稿 -> 拣货中）")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('wms:shipment-order:update')")
    public CommonResult<Boolean> startPick(@RequestParam("id") Long id) {
        shipmentOrderService.startPick(id);
        return success(true);
    }

    @PutMapping("/complete-pick")
    @Operation(summary = "完成拣货（拣货中 -> 已拣货）")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('wms:shipment-order:update')")
    public CommonResult<Boolean> completePick(@RequestParam("id") Long id) {
        shipmentOrderService.completePick(id);
        return success(true);
    }

    @PutMapping("/review")
    @Operation(summary = "复核（已拣货 -> 已复核）")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('wms:shipment-order:update')")
    public CommonResult<Boolean> review(@RequestParam("id") Long id) {
        shipmentOrderService.review(id);
        return success(true);
    }

    @PutMapping("/pack")
    @Operation(summary = "打包（已复核 -> 已打包）")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('wms:shipment-order:update')")
    public CommonResult<Boolean> pack(@RequestParam("id") Long id) {
        shipmentOrderService.pack(id);
        return success(true);
    }

    @PutMapping("/ship")
    @Operation(summary = "发货（已打包 -> 已发货）")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('wms:shipment-order:update')")
    public CommonResult<Boolean> ship(@RequestParam("id") Long id) {
        shipmentOrderService.ship(id);
        return success(true);
    }

    @PutMapping("/finish")
    @Operation(summary = "完成出库（已发货 -> 已完成，扣减库存）")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('wms:shipment-order:complete')")
    public CommonResult<Boolean> finish(@RequestParam("id") Long id) {
        shipmentOrderService.finish(id);
        return success(true);
    }
