...
import static cn.zhicloud.module.wms.enums.ErrorCodeConstants.INVENTORY_BATCH_PUTAWAY_EXPIRED;
import static cn.zhicloud.module.wms.enums.ErrorCodeConstants.SLOTTING_BATCH_MISMATCH;
import static cn.zhicloud.module.wms.enums.ErrorCodeConstants.SLOTTING_NO_SUITABLE_WAREHOUSE;
...
public class WmsSlottingServiceImpl implements WmsSlottingService {
    ...
    public Long mergeOrCreateBatch(WmsInventoryDO inventory, String batchNo, LocalDate productionDate,
        ...
        // 数量原子累加：quantity = quantity + 入库数量，避免并发覆盖
        int updatedRows = inventoryBatchMapper.increaseQuantity(existingBatch.getId(), quantity);
        if (updatedRows == 0) {
            throw exception(INVENTORY_BATCH_NOT_EXISTS);
        }
        log.info("[mergeOrCreateBatch][合并批次成功，batchId={} batchNo={} increasedQuantity={}]",
                existingBatch.getId(), batchNo, quantity);
