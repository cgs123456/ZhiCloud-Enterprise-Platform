...
import cn.zhicloud.module.wms.controller.admin.order.check.vo.order.WmsCheckOrderPageReqVO;
...
import cn.zhicloud.module.wms.enums.order.WmsCheckTypeEnum;
...
public class WmsCheckOrderServiceImpl implements WmsCheckOrderService {
    ...
    public Long createCheckOrder(WmsCheckOrderSaveReqVO createReqVO) {
        ...
        if (order.getCheckType() == null) {
            order.setCheckType(WmsCheckTypeEnum.OPEN.getType());
        }
        // 暗盘（BLIND）：系统自动生成空白盘点单，清空账面数量
        if (WmsCheckTypeEnum.BLIND.getType().equals(order.getCheckType()) && createReqVO.getDetails() != null) {
            createReqVO.getDetails().forEach(detail -> detail.setQuantity(null));
        }
