...
`yudao-module-ai-rag` 是 yudao 项目的可选模块（Spring Modulith 声明 A3），提供一套**完全本地化**的 RAG 能力：
...
- 通过 `yudao.airag.enabled=true` 开关控制启用，默认不启用（PostgreSQL + pgvector 未部署时不会影响主应用启动）

模块依赖 `yudao-module-ai`（复用 Spring AI 配置、`AiModelService`、ChatClient 等），并新增 `spring-ai-starter-vector-store-pgvector` 与 `spring-ai-tika-document-reader` 两个 Spring AI 依赖。
...
└── src/main/java/cn/zhicloud/yudao/module/airag/
    ├── AiragApplication.java      # 模块入口标记类（无独立 main，由 yudao-server 扫描加载）
    ├── package-info.java          # Spring Modulith 模块声明（A3，依赖 ai/system/infra）
    ├── config/
    │   └── AiragConfiguration.java     # 自动配置：@ConditionalOnProperty(yudao.airag.enabled=true)
    ├── enums/
...
> 控制器位于 `controller.admin` 包下，自动继承 yudao 框架的 `/admin-api` URL 前缀。
...
```bash
cd d:\Desktop\yudao\script\docker
...
```bash
psql -h localhost -U yudao -d yudao_rag -f d:\Desktop\yudao\sql\postgresql\airag_pgvector.sql
```
...
```
yudao-server/src/main/resources/airag/bge-base-zh/
...
在 `yudao-server` 的 `application-local.yaml`（或对应 profile）中：
...
yudao:
  airag:
    enabled: true
    datasource:
      url: jdbc:postgresql://localhost:5432/yudao_rag
      username: yudao
      password: yudao@2026
```
...
当前 `AiragConfiguration` 仅在容器中存在 `EmbeddingModel` Bean 时才创建 `VectorStore`。部署完成后需在 `AiragConfiguration` 中新增 `airagEmbeddingModel` Bean（参考 EMBEDDING_DECISION.md §4.2 方式 A），并补充 `spring-ai-transformers` 依赖（如未在 yudao-module-ai 中引入）。
...
### 7.1 与 yudao-module-ai 的关系

`yudao-module-ai` 已有基于 Redis / Qdrant / Milvus 的向量存储能力（通过 `AiKnowledgeService` + `AiModelService.getOrCreateVectorStore(modelId, ...)` 创建），其 Embedding 通常依赖外部 Embedding API（OpenAI 兼容 / Ollama 等）。

本模块定位为**完全本地化方案**，差异如下：

| 维度 | yudao-module-ai（已有） | yudao-module-ai-rag（本模块） |
|---|---|---|
...
| 启用方式 | 默认可用 | `yudao.airag.enabled=true` 显式启用 |
...
- `@ConditionalOnProperty`：未配置 `yudao.airag.enabled=true` 时整个配置类不加载
- `@ConditionalOnBean(EmbeddingModel.class)`：即使开关打开，未提供 `EmbeddingModel` Bean 也不创建 `VectorStore`

### 7.3 错误码段

使用 `1-041-xxx-xxx` 段，与 `yudao-module-ai`（`1-040`）和 `yudao-module-ai-multiagent`（`1-042`）区分：
