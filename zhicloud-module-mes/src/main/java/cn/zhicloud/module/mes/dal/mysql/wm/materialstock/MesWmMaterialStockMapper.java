package cn.zhicloud.module.mes.dal.mysql.wm.materialstock;
...
public interface MesWmMaterialStockMapper extends BaseMapperX<MesWmMaterialStockDO> {
     ...
     */
    @lombok.Data
    class ItemStockQuantitySummary {
        /**
         * 物料编号
         */
        private Long itemId;
    ...
    /**
     * 按物料汇总库存数量（SUM + GROUP BY 下推到 SQL，避免全量拉取内存聚合）
     *
     * @param itemIds 物料编号集合
     * @return 每个物料的汇总数量列表
     */
    @Select("<script>"
            + "SELECT item_id, SUM(quantity) AS total_quantity FROM mes_wm_material_stock "
            + "WHERE deleted = 0 AND item_id IN "
            + "<foreach collection='itemIds' item='itemId' open='(' separator=',' close=')'>#{itemId}</foreach> "
            + "GROUP BY item_id"
            + "</script>")
    List<ItemStockQuantitySummary> selectSumQuantityGroupByItemId(@Param("itemIds") Collection<Long> itemIds);
