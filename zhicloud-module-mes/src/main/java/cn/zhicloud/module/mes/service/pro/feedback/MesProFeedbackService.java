...
public interface MesProFeedbackService {
     ...
     * PDA 端简化报工：跳过审批直接完成
     *
     * <p>由 PDA 工位机扫描工单二维码后调用，根据 workOrderNo 反查工单与任务，自动填充以下字段：
     * <ul>
     *   <li>workOrderId / taskId / routeId / itemId / workstationId / itemId 来自关联任务</li>
     *   <li>报工时间取当前时间，报工类型为「自行报工」，报工途径为「PDA」</li>
     *   <li>状态直接置为 {@link cn.zhicloud.module.mes.enums.pro.MesProFeedbackStatusEnum#FINISHED}</li>
     * </ul>
     *
     * <p>简化流程不触发 IPQC / 产出单 / 物料消耗单，仅累加任务与工单的已生产数量。
     *
     * @param reqVO PDA 报工请求
     * @return 报工单编号
     */
    Long pdaSubmitFeedback(@Valid MesPdaFeedbackSubmitReqVO reqVO);

    /**
     * 查询指定用户的报工记录列表（PDA 端「我的报工」用）
     *
     * @param feedbackUserId 报工用户编号
     * @return 报工记录列表，按 id 倒序
     */
    java.util.List<MesProFeedbackDO> getFeedbackListByUser(Long feedbackUserId);
