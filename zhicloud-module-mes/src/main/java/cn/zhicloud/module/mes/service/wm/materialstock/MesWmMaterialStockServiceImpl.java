...
import cn.zhicloud.module.mes.controller.admin.wm.materialstock.vo.MesWmMaterialStockFreezeReqVO;
import cn.zhicloud.module.mes.controller.admin.wm.materialstock.vo.MesWmMaterialStockListReqVO;
import cn.zhicloud.module.mes.controller.admin.wm.materialstock.vo.MesWmMaterialStockPageReqVO;
...
import cn.zhicloud.module.mes.service.md.item.MesMdItemTypeService;
...
public class MesWmMaterialStockServiceImpl implements MesWmMaterialStockService {
    ...
    public java.util.Map<Long, BigDecimal> batchGetStockQuantity(java.util.Collection<Long> itemIds) {
        ...
        // SUM + GROUP BY 下推到 SQL，避免全量拉取库存记录后内存聚合
        Map<Long, BigDecimal> result = new HashMap<>();
        for (MesWmMaterialStockMapper.ItemStockQuantitySummary summary
                : materialStockMapper.selectSumQuantityGroupByItemId(itemIds)) {
            result.put(summary.getItemId(),
                    summary.getTotalQuantity() == null ? BigDecimal.ZERO : summary.getTotalQuantity());
        }
        return result;
    }
