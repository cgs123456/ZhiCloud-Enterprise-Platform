package cn.zhicloud.module.mes.service.pro.feedback;
...
import cn.zhicloud.module.mes.controller.admin.pro.feedback.vo.MesProFeedbackPageReqVO;
import cn.zhicloud.module.mes.controller.admin.pro.feedback.vo.MesProFeedbackSaveReqVO;
...
import cn.zhicloud.module.mes.dal.dataobject.wm.itemconsume.MesWmItemConsumeDO;
...
import cn.zhicloud.module.mes.dal.mysql.pro.feedback.MesProFeedbackMapper;
import cn.zhicloud.module.mes.enums.pro.MesProFeedbackStatusEnum;
import cn.zhicloud.module.mes.enums.pro.MesProFeedbackTypeEnum;
...
import cn.zhicloud.module.mes.service.wm.itemconsume.MesWmItemConsumeService;
import cn.zhicloud.module.mes.service.wm.productproduce.MesWmProductProduceService;
import cn.zhicloud.module.mes.service.wm.productproduce.MesWmProductProduceLineService;
import cn.zhicloud.module.mes.listener.MesProFeedbackApprovedEvent;
...
public class MesProFeedbackServiceImpl implements MesProFeedbackService {
    ...
    public boolean approveFeedback(Long id) {
        ...
        // 6. 发布报工审批通过事件（供计件工资等下游消费，AFTER_COMMIT 确保报工已落库）
        applicationEventPublisher.publishEvent(new MesProFeedbackApprovedEvent(id));
        return true; // 已完成
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Long pdaSubmitFeedback(MesPdaFeedbackSubmitReqVO reqVO) {
        // 1. 校验报工数量 > 0
        if (reqVO.getFeedbackQuantity() == null
                || reqVO.getFeedbackQuantity().compareTo(BigDecimal.ZERO) <= 0) {
            throw exception(PRO_PDA_FEEDBACK_QUANTITY_INVALID);
        }
        // 2.1 查询工单
        ...
        // 2.2 校验工单已确认
        workOrderService.validateWorkOrderConfirmed(workOrder.getId());

        // 3. 查询关联任务，取第一条匹配 processId 的任务
        List<MesProTaskDO> tasks = taskService.getTaskListByWorkOrderIds(java.util.Collections.singletonList(workOrder.getId()));
        MesProTaskDO task = tasks.stream()
                .filter(t -> ObjUtil.equal(t.getProcessId(), reqVO.getProcessId()))
                .filter(t -> !ObjUtil.equal(t.getStatus(), cn.zhicloud.module.mes.enums.pro.MesProTaskStatusEnum.FINISHED.getStatus()))
                .findFirst()
                .orElse(null);
        if (task == null) {
            throw exception(PRO_PDA_FEEDBACK_TASK_NOT_EXISTS);
        }

        // 4. 校验合格 + 不合格合计 = 报工数量（若工单工序非质检工序）
        MesProRouteProcessDO routeProcess = routeProcessService.getRouteProcessByRouteIdAndProcessId(
                task.getRouteId(), reqVO.getProcessId());
        boolean checkFlag = routeProcess != null && Boolean.TRUE.equals(routeProcess.getCheckFlag());
        BigDecimal qualified = ObjectUtil.defaultIfNull(reqVO.getQualifiedQuantity(), BigDecimal.ZERO);
        BigDecimal unqualified = ObjectUtil.defaultIfNull(reqVO.getUnqualifiedQuantity(), BigDecimal.ZERO);
        if (!checkFlag) {
            // 非质检工序：合格 + 不合格 = 报工数量
            if (qualified.add(unqualified).compareTo(reqVO.getFeedbackQuantity()) != 0) {
                throw exception(PRO_FEEDBACK_QUALIFIED_UNQUALIFIED_MISMATCH);
            }
        } else {
            // 质检工序：先以报工数量作为待检，合格/不合格后续回填
            reqVO.setQualifiedQuantity(null);
            reqVO.setUnqualifiedQuantity(null);
        }

        // 5. 构造报工单（直接已完成状态）
        LocalDateTime now = LocalDateTime.now();
        MesProFeedbackDO feedback = new MesProFeedbackDO()
                .setCode(generatePdaFeedbackCode(now))
                .setType(MesProFeedbackTypeEnum.SELF.getType())
                .setChannel("PDA")
                .setFeedbackTime(now)
                .setWorkstationId(task.getWorkstationId())
                .setRouteId(task.getRouteId())
                .setProcessId(reqVO.getProcessId())
                .setWorkOrderId(workOrder.getId())
                .setTaskId(task.getId())
                .setItemId(task.getItemId())
                .setFeedbackQuantity(reqVO.getFeedbackQuantity())
                .setQualifiedQuantity(checkFlag ? null : qualified)
                .setUnqualifiedQuantity(checkFlag ? null : unqualified)
                .setUncheckQuantity(checkFlag ? reqVO.getFeedbackQuantity() : BigDecimal.ZERO)
                .setFeedbackUserId(reqVO.getFeedbackUserId())
                .setStatus(MesProFeedbackStatusEnum.FINISHED.getStatus())
                .setRemark(reqVO.getRemark());
        feedbackMapper.insert(feedback);

        // 6. 简化流程：直接累加任务/工单已生产数量（跳过 IPQC / 产出单 / 物料消耗）
        //    质检工序的合格/不合格数量后续由 IPQC 完成时回调回写
        taskService.updateProducedQuantity(task.getId(),
                reqVO.getFeedbackQuantity(), qualified, unqualified);
        workOrderService.updateProducedQuantity(workOrder.getId(), reqVO.getFeedbackQuantity());

        // 7. 发布报工审批通过事件（供计件工资等下游消费，AFTER_COMMIT 确保报工已落库）
        applicationEventPublisher.publishEvent(new MesProFeedbackApprovedEvent(feedback.getId()));
        return feedback.getId();
    }

    @Override
    public List<MesProFeedbackDO> getFeedbackListByUser(Long feedbackUserId) {
        if (feedbackUserId == null) {
            return java.util.Collections.emptyList();
        }
        return feedbackMapper.selectList(MesProFeedbackDO::getFeedbackUserId, feedbackUserId);
    }

    /**
     * 生成 PDA 报工单编号：PDA + yyyyMMddHHmmss + 4 位随机数
     */
    private String generatePdaFeedbackCode(LocalDateTime time) {
        return "PDA" + time.format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"))
