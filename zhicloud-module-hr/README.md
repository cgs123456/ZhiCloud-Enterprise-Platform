...
`zhicloud-module-hr` 是 zhicloud 项目的业务模块（Spring Modulith 声明），提供一套面向中小企业的核心人力资源管理能力：
...
模块所有表均继承 zhicloud 框架 `TenantBaseDO`，支持多租户隔离。
...
zhicloud-module-hr/
├── pom.xml                        # 模块依赖声明
└── src/main/java/cn/zhicloud/zhicloud/module/hr/
...
> 控制器位于 `controller.admin` 包下，自动继承 zhicloud 框架的 `/admin-api` URL 前缀，所有写操作均需 `@PreAuthorize` 权限校验。
...
> 建表脚本：`sql/mysql/hr.sql`，所有表均含 `creator/create_time/updater/update_time/deleted/tenant_id` 字段（继承 zhicloud 框架 `TenantBaseDO`）。
...
### 7.1 与 `zhicloud-module-erp`（计划联动）
...
### 7.2 与 `zhicloud-module-system`（用户体系）

员工档案 `hr_employee` 可关联系统用户 `system_users`（通过 userId 字段，当前 MVP 未实现），实现：
- 员工自助查询考勤/薪资
- 登录账号与员工身份绑定

### 7.3 与 `zhicloud-module-infra`（基础设施）

复用 zhicloud 框架能力：
- 多租户（`zhicloud-spring-boot-starter-biz-tenant`）
...
| 薪酬凭证联动 | 对接 `zhicloud-module-erp` 总账，自动生成薪酬凭证 |
...
- zhicloud-module-system（用户体系）：见项目根目录
